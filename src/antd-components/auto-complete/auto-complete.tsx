...
export default function Autocomplete(props: any) {
  const { requestFunc, highlightStatus = false, delayTime = 0, currentShow = false,extendParams={} } = props;
  const [data, setData] = useState<any>([]);
  const [options] = useState<any>(props.options || []);
  const [keyword, setKeyword] = useState<string>('');
  useEffect(() => {
    requestFunc && requestFunc({ value: keyword,...extendParams}).then((res: any) => {
  ...
  const children = options.length===0 && data.map((item: any, index: string) =>
